...
def read_csv_to_df():
    ...
    movies["tcast"] = movies["top_cast"].apply(
        lambda x: [i.replace(" ", "") for i in x]
    )
    movies["tcrew"] = movies["director"].apply(
        lambda x: [i.replace(" ", "") for i in x]
    )
    movies["tprduction_comp"] = movies["prduction_comp"].apply(
        lambda x: [i.replace(" ", "") for i in x]
    )
...
@st.cache_data
def load_similarity_matrix(pickle_file_path):
...
def recommend(new_df, movie, pickle_file_path):
    similarity_tags = load_similarity_matrix(pickle_file_path)

    movie_idx = new_df[new_df["title"] == movie].index[0]

    # Getting the top 25 movies from the list which are most similar
    movie_list = sorted(
        list(enumerate(similarity_tags[movie_idx])), reverse=True, key=lambda x: x[1]
    )[1:26]
...
def fetch_person_details(id_):
    try:
        response = requests.get(
            f"https://api.themoviedb.org/3/person/{id_}?api_key=6177b4297dff132d300422e0343471fb",
            timeout=5
        )
        data = response.json()
...
@st.cache_data
def load_movies_dicts():
    with open(r"Files/movies_dict.pkl", "rb") as f1:
        movies = pd.DataFrame.from_dict(pickle.load(f1))
    with open(r"Files/movies2_dict.pkl", "rb") as f2:
        movies2 = pd.DataFrame.from_dict(pickle.load(f2))
    return movies, movies2


@st.cache_data
def get_details(selected_movie_name):
    movies, movies2 = load_movies_dicts()
